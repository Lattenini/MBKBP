...
public:
    FastBB(int **Graph, int Graph_size, int Bipartite_index,int *degree, int delta, int k,int threshold_l,int threshold_r,int high_l,string version,int spenode,int *G_label,int enuNum);
    //baseline version
    FastBB(int **Graph, int Graph_size, int Bipartite_index,int *degree, int delta, int k,int threshold_l,int threshold_r,int high_l,string version,int spenode);
    ~FastBB();
    void BKmb();
    int MaxNodeNum;
    int MaxEdgeNum;
...
FastBB::FastBB(int **Graph, int Graph_size, int Bipartite_index,int *degree, int delta, int k,int threshold_l,int threshold_r,int high_l,string version,int spenode,int *G_label,int enuNum)
{
    this->Graph=Graph;
    this->degree=degree;   
...
FastBB::FastBB(int **Graph, int Graph_size, int Bipartite_index,int *degree, int delta, int k,int threshold_l,int threshold_r,int high_l,string version,int spenode)
{
    this->Graph=Graph;
    this->degree=degree;   
    this->graph_size=Graph_size;
...
void FastBB::BKmb(){
    bool flag;
    //问题三：
    if(version == "spenode" || version == "speEdge" ){//特殊顶点来自于左侧
        if(spenode<bi_index){
            P_L.add(spenode);
            //Cand_R第一次更新
            for(temp_i=degree[spenode]-1;temp_i>=0;--temp_i){
                temp_node=Graph[spenode][temp_i];//i的邻居
                if(degree[temp_node]<left-k) continue;
                //更新度数信息
            ...
            //Cand_L第一次更新
            for(temp_i=degree[spenode]-1;temp_i>=0;--temp_i){
                temp_node=Graph[spenode][temp_i];
                for(temp_j=degree[temp_node]-1;temp_j>=0;--temp_j){
                    temp_node2=Graph[temp_node][temp_j];
                    if(temp_node2 == spenode) continue;
                    if(degInG[temp_node2]>=right-2*k&&Cand_L.vpos[temp_node2]==graph_size){//感觉&&Cand_L.vpos[temp_node2]==graph_size可以删除
            ...
            for(temp_i=degree[spenode]-1;temp_i>=0;--temp_i){
                temp_node=Graph[spenode][temp_i];//i的邻居
                if(degree[temp_node]<right-k) continue;
                //更新度数信息
                degInG[temp_node]++;//因为i一定会加入，所以把temp_node度数+1
                degInP[temp_node]++;
                Cand_L.add(temp_node);
                for(temp_j=degree[temp_node]-1;temp_j>=0;--temp_j){//temp_node此时加入了Cand_R，所以更新temp_node邻居的度数
                    temp_node2=Graph[temp_node][temp_j];
                    degInG[temp_node2]++;
                }
            ...
            for(temp_i=degree[spenode]-1;temp_i>=0;--temp_i){
                ...
                for(temp_j=degree[temp_node]-1;temp_j>=0;--temp_j){
                    temp_node2=Graph[temp_node][temp_j];
                    if(temp_node2 == spenode) continue;
                    if(degInG[temp_node2]>=left-2*k&&Cand_R.vpos[temp_node2]==graph_size){//感觉&&Cand_R.vpos[temp_node2]==graph_size可以删除
                        Cand_R.add(temp_node2);
                        for(temp_k=degree[temp_node2]-1;temp_k>=0;--temp_k){
                            degInG[Graph[temp_node2][temp_k]]++;//更新度数：此时temp_node2加入了Cand_L,更新其邻居的度数
                        }
                    }
            ...
            for(temp_i=0;temp_i<bi_index;++temp_i){
                if(degInG[temp_i]>=right-k&&Cand_L.vpos[temp_i]==graph_size){//新加入:之前只考虑了i的邻居，这里补齐符合条件的非邻居
                    Cand_L.add(temp_i);
                    for(temp_j=degree[temp_i]-1;temp_j>=0;--temp_j){
                        degInG[Graph[temp_i][temp_j]]++;
                    }
                }
                if(degInG[temp_i]<right-k&&Cand_L.vpos[temp_i]<graph_size){//不符合条件的去除(肯定是i的邻居，之前假设i后面所有顶点都在，其实并不是)
                    Cand_L.remove(temp_i);
                    //恢复一些度数
                    for(temp_j=degree[temp_i]-1;temp_j>=0;--temp_j){
                        temp_node=Graph[temp_i][temp_j];
                        degInG[temp_node]--;
                        if(degInG[temp_node]<left-k&&Cand_L.vpos[temp_node]<graph_size){
                            //再次更新Cand_R
...
void FastBB::BKmb_Rec(){
    // ||后面是P2 BBB
    // if(P_L.vnum>=high_l||P_R.vnum>=high_l+delta){//high_l是左侧顶点的数量上界 !!!!!!!baseline solution !!!!!! 222
    //     return;
    // }
    ...
    if(version == "node" || version == "spenode"){
        //以下为BBB
        //不是顶点最多
    ...
    }else if(version == "edge" || version == "speEdge"){
        //不是边最多1
        if((P_L.vnum+Cand_L.vnum)*(P_R.vnum+Cand_R.vnum)<=MaxEdgeNum&&(P_L.vnum+Cand_L.vnum)*(P_R.vnum+Cand_R.vnum)>0) 
            return;
        //不是边最多2
        temp_j=0;
        for(temp_i=P_L.vnum-1;temp_i>=0;--temp_i){
        ...
        if(temp_j<=MaxEdgeNum){//P_L和Cand_L中所有顶点的度数之和<MaxEdgeNum
            return;
        }
    }
    ...
    if(Cand_R.empty()&&Cand_L.empty()){
        if(Exc_R.empty()&&Exc_L.empty() ){
            if(version == "node" || version == "spenode"){
                ...
                if(temp_j>MaxEdgeNum && checkBalance()  ){
                    MaxEdgeNum = temp_j;
                    recordRes();
                }
            }
                ...
                for(temp_i=Cand2->vnum-1;temp_i>=0;--temp_i){
                    ...
                    if(degInP[temp_node]<temp_size2-k||degInG[temp_node]<temp_stand1-k){//不符合条件的
                        ...
                        if(Cand2->contains(temp_node)){//如果通过上面这个for的条件，继续在这里看能否通过共同邻居的条件剪枝
                            ...
                            if(count<temp_stand1-2*k ){//用的是共同邻居+2k的剪枝条件S3 ||count<=(MaxNodeNum-delta)/2-2*k
                                Cand2->remove(temp_node);
                                remove_C2.push_back(temp_node);
                                for(temp_j=degree[temp_node]-1;temp_j>=0;--temp_j){
                                    temp_node2=Graph[temp_node][temp_j];
        ...
        if(temp_value<=k){//pivot的非邻居应该是不能<=k的;temp_value代表所有顶点的最大非邻数量，这时候如果所有顶点非邻居都<=k，就可以开始判断结果了
            if(Exc_L.empty()&&Exc_R.empty()){
                if(version == "node" || version == "spenode"){
                    ...
                    if(temp_value>MaxEdgeNum){
                        if(checkBalance()){
                            MaxEdgeNum = temp_value;
                            recordRes();
                        }
                    }
                }
...
bool FastBB::CanBranch(){//检查当前分支是否是极大的，他的下一部分是在B0处
    temp_j=P_L.vnum+Cand_L.vnum;
    temp_k=P_R.vnum+Cand_R.vnum;
    for(temp_i=Exc_L.vnum-1;temp_i>=0;--temp_i){
        temp_node=Exc_L.vlist[temp_i];
...
void FastBB::recordRes(){
    ...
    for(int i=P_L.vnum-1;i>=0;--i){
        if(version == "spenode" || version == "speEdge"){
            Res_L.push_back(re_temp_index_spe[P_L.vlist[i]]); 
        }else{
            // Res_L.push_back(re_temp_index[P_L.vlist[i]]); //BBB
            Res_L.push_back(P_L.vlist[i]); 
        }
    }    

    for(int i=Cand_L.vnum-1;i>=0;--i){
        if(version == "spenode" || version == "speEdge"){
            Res_L.push_back(re_temp_index_spe[Cand_L.vlist[i]]);   
        }else{
            // Res_L.push_back(re_temp_index[Cand_L.vlist[i]]); //BBB
            Res_L.push_back(Cand_L.vlist[i]); 
        }
    }    
    //right side
    for(int i=P_R.vnum-1;i>=0;--i){
        if(version == "spenode" || version == "speEdge"){
            Res_R.push_back(re_temp_index_spe[P_R.vlist[i]]);
        }else{
            // Res_R.push_back(re_temp_index[P_R.vlist[i]]);//BBB
            Res_R.push_back(P_R.vlist[i]);
        }
    }
    for(int i=Cand_R.vnum-1;i>=0;--i){
        if(version == "spenode" || version == "speEdge"){
            Res_R.push_back(re_temp_index_spe[Cand_R.vlist[i]]);
        }else{
            // Res_R.push_back(re_temp_index[Cand_R.vlist[i]]);//BBB
            Res_R.push_back(Cand_R.vlist[i]);
        }
    }
    if(version == "node" || version == "spenode"){
        cout  << "更新顶点最大: " << MaxNodeNum << endl;
        cout  << "|L|: " << Res_L.size() << endl;
        cout  << "|R|: " << Res_R.size() << endl;
    }else if(version == "edge" || version == "speEdge"){
        cout  << "更新边最大: " << MaxEdgeNum << endl;
        cout  << "|L|: " << Res_L.size() << endl;
        cout  << "|R|: " << Res_R.size() << endl;
    }
    cout  << "Res_L:" ;
...
uint32_t FastBB::biplexUpperBound(uint32_t u,int sizeP1,int sizeP2,RandList *P2,int sizeC1,RandList *C1) //u将加入P2
{
...
int FastBB::binarySearchUB(int low,int high,vector<int> numbers,int k,int delta){
    int max=0;
    while(low<=high){
        int mid ;
        mid = (high+low)/2;
...
int FastBB::binarySearchUB2(int low,int high,vector<int> numbers,int k,int delta){
    int max=0;
    while(low<=high){
        int mid ;
        mid = (high+low)/2;
...
void FastBB::greedyAdd(vector<int>Np, RandList *&Cand1, RandList *&Cand2,RandList *&P1,RandList *&P2,RandList *&Exc1,RandList *&Exc2,vector<int>&remove_C1, vector<int>&remove_E1, vector<int>&remove_C2, vector<int>&remove_E2,int temp_stand1,int temp_stand2){
    for(int i: Np){
        //P中加入i
        P1->add(i);
        Cand1->remove(i);
            ...
            for(temp_i=Cand1->vnum-1;temp_i>=0;--temp_i){
                ...
                if(degInP[temp_node]<temp_size2-k||degInG[temp_node]<temp_stand2-k){//不符合条件的
                    ...
                    if(Cand1->contains(temp_node)){//如果通过上面这个for的条件，继续在这里看能否通过共同邻居的条件剪枝
                        ...
                        //24.10.12日更新：MaxNodeNum本来应该只出现在node version中的，因为后续只做node的实验，所以暂时就这么写了更方便
                        if(count<temp_stand2-2*k){//用的是共同邻居+2k的剪枝条件S3  ||count<=(MaxNodeNum-delta)/2-2*k
                            Cand1->remove(temp_node);
                            remove_C1.push_back(temp_node);
                            for(temp_j=degree[temp_node]-1;temp_j>=0;--temp_j){
                                temp_node2=Graph[temp_node][temp_j];
